...
3. **결정론적 설치 실행.** 승인(대화형) 또는 `--yes`(비대화형) 후,
   ...
   로컬 편집 감지 시 **skip + diff 표시**, 조용한 overwrite 금지. 반환 dict의 `loop_tracked_deleted`에 경로가 있으면(커밋된 루프 파일이 워킹트리에서만 지워짐) 새로 안 쓰고 사용자에게 "복구할지 정하라"고 알린다. ⚠️ `settings.json`이 JSONC면
...
⚠️ **고아 제거와 링크 교정은 독립 작업이 아니다 — 순차 의존이다.** 고아 문서를 인덱스에 등록하는
순간 gate의 BFS가 **그 문서 안으로 걸어 들어가**, 지금까지 고아-됨에 가려져 있던 내부 깨진 링크가
새로 드러난다. 실측: 고아 16개를 등록하자 `broken`이 8 → 18로 늘었다(전부 선존). **de-link 2차
패스를 예산에 넣어라.** "고아 0을 만들었더니 broken이 늘었다"는 퇴행이 아니라 정상이다.
...
- **타겟-소실**(링크 대상이 삭제/이동됨): ①현행 문서 + 살아있는 등가물 존재 → repoint. ②**동결 역사
  ...
  ⚠️ de-link는 **`](` 토큰을 없애는 것**이다(`` `라벨` `` 또는 `` `경로` ``). 링크 *전체*를 백틱으로
  감싸는 것(`` `[라벨](경로)` ``)은 de-link가 아니라 **코드 예시**이고, gate는 코드스팬을 링크로 세지
  않으므로 그것도 통과하지만 — 원래 문장이 링크였다면 예시로 바꾸지 마라.
