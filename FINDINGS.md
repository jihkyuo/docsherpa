...
- **⚠️ 미해결 갱 2건(실험이 표면화, 아직 안 고침):**
  1. `scaffold.install_loop_files`는 `not dst.exists()`(**워킹트리**)로 판정한다. git에 추적되지만
     워킹트리에서 지워진 파일은 "새 설치"로 취급 → 사용자의 **커밋된 루프를 diff-preview 없이 조용히
     버전업**한다. R4 정책("덮지 않는다")의 사각.
  2. 경량 차선에서 **고아를 없애면 깨진 링크가 새로 드러난다** — 인덱스에 등록하는 순간 BFS가 그
     문서로 걸어 들어가기 때문. SKILL은 두 작업을 독립으로 취급하지만 실제로는 순차 의존이다.
- **▶️ 다음:** 앵커를 "참고 목록"이 아니라 **명시적 체크리스트**(항목마다 해당됨/해당없음 판정 강제)로
  바꾸고, **같은 시나리오로 재측정**한다. 재현율이 오르면 채택, 안 오르면 폐기. 코드 0줄.
  실패 모드가 "내용 부재"가 아니라 "주의 분산"임이 실측이다 — 두 SKILL 모두 같은 규칙을 담았는데
  둘 다 흔들렸다.
