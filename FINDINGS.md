...
- **🧪 parity 실험 1차 결과(2026-07-10) — 핵심 가설은 반증되지 않았다. 그러나 루프 자체가 흔들린다.**
  절차·재현법은 [how-to/run-parity-experiment.md](docs/how-to/run-parity-experiment.md).
  시나리오 = 코드 착륙 후 문서만 정리한 커밋. 정답지 = `Docs-Impact:` 트레일러 + 실제 변경 파일.
  대조군(손-작성 라우터 + 원본 doc-reconcile) vs 처리군(벗긴 뒤 `setup-docs`가 생성한 루프), **각 3회.**
  ```
  탐지 재현율   대조군 67%  ·  처리군 50%      ← 팔 사이 차이 < 팔 내부 변동. n=1로 단정 금지.
  억제(신설 안 함)  3/3    ·    3/3           ← 견고
  보존(안 건드림)   3/3    ·    3/3           ← 견고
  ```
  **읽는 법:** 사용자가 체감하는 가치는 **탐지가 아니라 억제·보존**에서 온다. 원본 루프도 3회 중
  2회는 목표 하나를 놓쳤다 — "잘 돈다"는 체감은 *안 해도 될 일을 안 해서* 생긴 것이다.
  **설치본 기반은 잘 재생된다:** 앵커 3/3이 손-작성과 같은 타겟을 짚었고(잃은 건 도메인 리터럴 1개),
  차선 선택도 정확했으며(이동 0), 도달성은 **처리군이 더 낫다**(원본 레포는 orphan=15, 자기 불변식 위반).
  **소실된 것:** 인덱스의 *큐레이션*(원본 13줄의 "이건 왜 먼저 읽어라" 설명) → 도달은 되나 안내가 없다.
- **🐛 실험이 찾은 진짜 버그(PR #12):** `gate.py`가 **인라인 코드스팬 안의 링크를 live로 셌다.**
  링크 문법을 예시로 담은 동결 문서는 예시를 훼손해야만 `broken=0`에 도달 — 내용 소실 0과 충돌.
  1차 픽스는 백틱 런 길이를 안 봐서 **거짓 음성**을 넣었고 적대적 리뷰가 잡았다(CommonMark §6.3).
- **✅ 실험이 표면화한 갱 2건 — Phase 0에서 닫음:**
  1. `scaffold.install_loop_files`가 **워킹트리 존재만** 보고 판정해, git에 추적되지만 지워진 루프
     파일을 "새 설치"로 취급 → 커밋된 루프를 **diff-preview 없이 조용히 버전업**했다(R4 우회).
     → HEAD 추적 확인 추가, 추적 중이면 **쓰지 않고 `loop_tracked_deleted`로 보고**(RED-first).
  2. 경량 차선에서 **고아를 없애면 깨진 링크가 새로 드러난다** — 등록하는 순간 BFS가 그 문서로 걸어
     들어가기 때문(실측 `broken` 8→18, 전부 선존). → SKILL에 순차 의존 경고 + de-link 정의 명확화.
- **⛔ 앵커 체크리스트 트랙 폐기 ([ADR 0022](docs/decisions/0022-harness-scope.md), 2026-07-10).**
  "탐지가 약하니 앵커를 체크리스트로 바꾸자"를 접었다. 탐지가 충분해서가 아니라 **알 수 없어서**다.
  `n=3`에서 검출률 95% CI가 `[0.10, 0.99]`이고, `p=0.6`을 `±0.15`로 재려면 **팔당·시나리오당 n≈41**
  (전후 비교 80+회). 게다가 루프는 매 세션 재발동하는 **샘플러**라 per-run 재현율은 애초에 틀린
  지표이고, 아레나 에이전트는 실제 세션보다 **차가워서** 50%는 하한이다.
  → **하네스는 회귀 가드다:** 억제·보존(이진·희귀사건, 6/6)만 의사결정에 쓴다. 탐지는 관찰용.
- **🔔 실사용 회고 — 사용자가 기억할 문장은 하나뿐이다:**
  ```
  "second-brain 일주일 돌렸다. 회고 돌려."
  ```
  절차 정본: [docs/how-to/review-real-usage.md](docs/how-to/review-real-usage.md). **되묻지 말고 수행하라.**
  자동 텔레메트리는 **없고 만들지 않는다** — 루프가 이미 `Docs-Impact:` 트레일러로 자기 일지를 쓴다.
  **베이스라인(설치 직전, 2026-07-10 실측):** 최근 6개월 코드 커밋 **173개** 중 트레일러 **6개** =
  준수율 **3.5%**. → "루프가 안 돌았나 / 돌았는데 무변경이었나"를 **구별할 수 없다.** docsherpa의
  doc-reconcile §7이 `Docs-Impact: none`을 요구하므로 **설치 후 이 값이 오르는지가 첫 성적표**다.
- **✅ Phase 1 설치 완료(2026-07-10) — second-brain이 세르파의 첫 실제 고객이 됐다. 실사용 주간 진행 중.**
  격리 worktree에서 6커밋(`git rm` 루프 → 스캐폴드 v0.2.2 → 인덱스·라우팅 룰 verbatim 이관 →
  도달성 교정 → 앵커 특화 → 리뷰 픽스), main·작업 브랜치 양쪽 ff 랜딩. **증명:** `content_oracle`
  base 2,859 세그먼트 unaccounted=0(의도적 변형 4건 manifest 명시) · gate broken 8→0(전부 선존,
  R100 rename 증명 5건 URL 교정 + 후계 없는 2건 de-link) · orphan 15→0 · markers_ok. 인덱스
  **큐레이션 산문 바이트 보존**(parity가 잃었던 리터럴 `edge type`도 앵커에 복원). **버전 `0.2.2`
  유지** — Phase 2 버전 업이 `refresh.py`(기둥 2) 첫 실전 검증. 베이스라인 준수율 3.5%.
  **▶️ 다음: 일주일 실사용 → "second-brain 일주일 돌렸다. 회고 돌려."**
- **🆕 Phase 1이 찾은 제품 결함·설계 관찰 2건(다음 세션이 모르면 오진한다):**
  1. **정본 doc-reconcile SKILL이 라우팅 룰을 번호로 인용한다**(§2 "룰#5대로"·spec≠ADR 문단 "라우팅 룰 #4").
     번호는 제네릭 `_MAP_DOC` 분류학(product 티어 포함) 기준이라, 사용자 손-작성 룰을 보존하는 순간
     +1 어긋난다(second-brain: specs=#3인데 #4로 인용 — 리뷰 파인더 3개가 독립 확증). 설치본은 이름
     인용으로 교정했으나 **정본 픽스는 루프 산문 변경 = parity 재측정 필요(ADR 0022)** → Phase 2 후보.
  2. **§7.5 앵커 특화 = 로컬 편집 → 특화된 설치본은 refresh가 영구 `stuck`.** `canonical_hash`가
     앵커 블록을 제외하지 않으므로 스탬프 sha(제네릭 본문 해시)와 영원히 불일치. **Phase 2 버전 업
     때 `refreshed`가 아니라 `stuck`이 뜨는 게 정상이다 — 버그로 오진하지 마라.** 조용한 덮어쓰기
     방지라는 점에선 옳은 보수 동작이나, 기둥 2가 특화 설치본에 사실상 무력하다는 설계 함의.
- **🔭 회고 감시 항목(주간 관찰, 손대지 말 것):** ⓐ 새 prime은 프로토타입의 절제 프로토콜(침묵
  장부·한 줄 제안·승낙 게이트)보다 무뚝뚝 — 억제·보존 축이 정확히 이걸 잰다(하네스 6/6은 통과 상태).
  ⓑ second-brain CONCEPTS의 "complex 3개 vs config 4개" 드리프트가 살아 있다면 루프가 잡는지 —
  정본이 이 교육 예시를 제네릭화하며 잃었다(도메인 리터럴 0 가드의 대가).
