...
- **▶️ 다음(Phase 1):** 프로토타입 루프를 `git rm`+커밋으로 제거 → `setup-docs` 실행(손-작성 인덱스·
  라우팅 룰을 `_map.md`로 **비파괴 이관** — 마커가 없어 `migrate_inline_to_map`은 no-op이라 에이전트
  판단 + `content_oracle` 무손실 증명) → 일주일 실사용 → 회고. **버전은 `0.2.2` 유지** — Phase 2의
  버전 업이 `refresh.py`(기둥 2 = 버전관리 업데이트)의 **첫 실전 검증**이 된다.
